...
        
        if status in tickets_by_status_count:
            tickets_by_status_count[status] += 1
        else:
            tickets_by_status_count['Desconhecido'] += 1
...
    
    # Cálculo correto da taxa de resposta
    responded_tickets_count = tickets_by_status_count.get('Respondido', 0) + tickets_by_status_count.get('Resolvido', 0)
    response_rate = (responded_tickets_count / total_tickets) * 100 if total_tickets > 0 else 0
    
    # Distribuição de Status
    ticket_status_distribution = {}
    for status, count in tickets_by_status_count.items():
        # Novo: Garante que o status 'Desconhecido' não entre no gráfico.
...
        users_data = {doc.id: doc.to_dict() for doc in users_ref.stream() if doc.id in users_with_tickets}
        
