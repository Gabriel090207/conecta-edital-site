...
    db_firestore_client = firestore.client()
    monitorings_ref = db_firestore_client.collection('monitorings').where('user_uid', '==', user_uid)
...
    if not user_email:
        raise HTTPException(
            status_code=404, detail="E-mail do usuário não encontrado."
        )
...
async def list_all_tickets(user_uid: str = Depends(get_current_user_uid)):
...
async def get_ticket_details(ticket_id: str, user_uid: str = Depends(get_current_user_uid)):
    """Retorna os detalhes de um único ticket, incluindo as mensagens."""
    db = firestore.client()
...
@app.get("/admin/stats")
async def get_admin_stats(user_uid: str = Depends(get_current_user_uid)):
    db = firestore.client()
...
@app.get("/admin/users")
async def get_all_users_for_audit(user_uid: str = Depends(get_current_user_uid)):
    """
