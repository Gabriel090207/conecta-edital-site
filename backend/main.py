...
    response_rate = (response_count / total_tickets) * 100 if total_tickets > 0 else 0
    
...
        user_email = ticket.to_dict().get('user_email')
        if user_uid:
            tickets_by_user[user_uid] += 1
            if user_email:
                user_emails[user_uid] = user_email
    
    most_active_users = []
    for uid, count in sorted(tickets_by_user.items(), key=lambda item: item[1], reverse=True)[:5]:
        user_doc = users_ref.document(uid).get()
        user_data = user_doc.to_dict() if user_doc.exists else {}
        user_name = user_data.get('fullName', 'Usuário Desconhecido')
        user_email = user_emails.get(uid, 'email@desconhecido.com')
...
        })
        
    # Usuários Ativos (com base em monitoramentos, como na rota `/admin/stats`)
...
        "satisfaction_rate": 0, # Fictício por enquanto, pois não temos dados de satisfação
